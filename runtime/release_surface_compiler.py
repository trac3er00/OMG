...
def _install_intro_content() -> str:
    return (
        ...
        "```\n"
        "\n"
        "If you run `npm install @trac3er/oh-my-god`, it resolves dependencies and links "
        "`omg` into `node_modules/.bin/` but makes no configuration mutations. For one-off "
        "or CI use, `npx omg ...` downloads and runs without a permanent local install.\n"
        "\n"
