...
def evaluate_release_compliance(
    *,
    ...
    if not isinstance(release_evidence, dict):
        return {"status": "allowed", "authority": "release", "reason": "release evidence unavailable"}
    ...
    return {"status": "allowed", "authority": "release", "reason": "compliance checks passed"}


def _artifact_gate_decision(release_evidence: dict[str, object] | None) -> dict[str, object]:
    if not isinstance(release_evidence, dict):
        return {"status": "allowed", "authority": "artifact", "reason": "no artifact supplied"}
    artifact = release_evidence.get("artifact")
    if not isinstance(artifact, dict):
        return {"status": "allowed", "authority": "artifact", "reason": "no artifact supplied"}
    verified = verify_artifact(artifact)
    action = str(verified.get("action", "allow")).strip().lower()
    if action == "deny":
        return {
            "status": "blocked",
            "authority": "artifact",
