...
- `hooks/_common.py` already has `get_project_dir()` at line ~17 that does the raw `os.environ.get("CLAUDE_PROJECT_DIR", os.getcwd())`. T1 must ADD a NEW `_resolve_project_dir()` function that validates `.oal/` existence, distinct from the existing function.
...
- OAL is stdlib-only — NO external dependencies ever.
