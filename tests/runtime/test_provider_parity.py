...
from runtime.host_parity import evaluate_provider_registry_parity, get_provider_parity_stubs
...
def test_confirmed_providers() -> None:
    ...
    assert confirmed_names == {"claude", "codex", "gemini", "kimi"}


def test_pending_stubs() -> None:
    pending = get_pending()

    assert {provider.name for provider in pending} == {"qwen-cli", "kilocode", "conductor"}
...
def test_parity_report(monkeypatch) -> None:
    monkeypatch.setattr(ProviderEntry, "is_available", lambda self: self.name in {"claude", "codex"})
...
def test_host_parity_registry_evaluation_covers_all_providers(monkeypatch) -> None:
    monkeypatch.setattr(ProviderEntry, "is_available", lambda self: self.name != "conductor")
