"""TDD tests for runtime.memory_parsers.kimi_import — Kimi Web paste-based memory import."""
...
from runtime.memory_parsers.kimi_import import (
    EXTRACTION_PROMPT,
    import_from_paste,
    parse_kimi_paste,
)
...
class TestParseKimiPaste:
    """Test parse_kimi_paste function."""

    def test_parse_kimi_paste_bullet_points(self) -> None:
        """Should parse bullet point format."""
        text = "- User prefers dark mode\n- Likes Python\n- Works on AI projects"
        result = parse_kimi_paste(text)
        assert len(result) == 3
    ...
    def test_parse_kimi_paste_numbered_list(self) -> None:
    ...
    def test_parse_kimi_paste_empty_input(self) -> None:
        """Should return empty list for empty input."""
        result = parse_kimi_paste("")
        assert result == []

    def test_parse_kimi_paste_whitespace_only(self) -> None:
        """Should return empty list for whitespace-only input."""
        result = parse_kimi_paste("   \n\n  \t  ")
        assert result == []

    def test_parse_kimi_paste_correct_source_cli(self) -> None:
        """Should set source_cli to 'kimi-web'."""
        text = "- Memory item"
        result = parse_kimi_paste(text)
        assert len(result) == 1
        assert result[0]["source_cli"] == "kimi-web"

    def test_parse_kimi_paste_correct_tags(self) -> None:
        """Should include 'kimi-web' and 'imported' in tags."""
        text = "- Memory item"
        result = parse_kimi_paste(text)
        assert len(result) == 1
        assert "kimi-web" in result[0]["tags"]
        assert "imported" in result[0]["tags"]

    def test_parse_kimi_paste_correct_key_format(self) -> None:
        """Should use 'kimi-memory-{i+1}' format for keys."""
        text = "- First\n- Second\n- Third"
        result = parse_kimi_paste(text)
        assert result[0]["key"] == "kimi-memory-1"
        assert result[1]["key"] == "kimi-memory-2"
        assert result[2]["key"] == "kimi-memory-3"

    def test_parse_kimi_paste_mixed_formats(self) -> None:
    ...
    def test_parse_kimi_paste_strips_whitespace(self) -> None:
...
class TestImportFromPaste:
    ...
    def test_import_from_paste_uses_correct_parameters(self) -> None:
        ...
        assert call_args.kwargs["source_cli"] == "kimi-web"
        assert "kimi-web" in call_args.kwargs["tags"]
