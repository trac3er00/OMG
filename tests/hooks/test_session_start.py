...
def test_session_start_uses_oal_state_paths():
    """SessionStart should point to .oal canonical paths."""
    with open("hooks/session-start.py") as f:
        content = f.read()

    assert ".oal/state" in content
...
def test_handoff_filename_contract():
    """Verify session-start reads handoff.md (not OAL:handoff.md)."""
    with open("hooks/session-start.py") as f:
        content = f.read()

    # Should look for handoff.md
    assert '"handoff.md"' in content
    # Should NOT look for OAL:handoff.md
    assert "OAL:handoff.md" not in content
...
def test_session_start_does_not_advertise_legacy_omc_aliases_by_default():
    with open("hooks/session-start.py") as f:
        content = f.read()
    assert "OAL_INCLUDE_LEGACY_ALIASES" in content
    assert "omc-teams" not in content.split("OAL_INCLUDE_LEGACY_ALIASES", 1)[0]
...
def test_session_start_missing_state_dir_exits_zero(tmp_path):
    """With no .oal directory at all, session-start should still exit 0."""
    proc = _run_session_start(tmp_path)
    assert proc.returncode == 0
...
def test_memory_injected_when_files_exist(tmp_path):
    ...
    env = {**os.environ, "CLAUDE_PROJECT_DIR": str(tmp_path), "OAL_MEMORY_ENABLED": "true"}
    proc = subprocess.run(
        [sys.executable, str(HOOK)],
        input=json.dumps({}),
...
def test_memory_not_injected_when_no_dir(tmp_path):
    """Without .oal/state/memory/, no @recent-memory: should appear."""
...
def test_memory_not_injected_when_flag_disabled(tmp_path):
    ...
    env = {**os.environ, "CLAUDE_PROJECT_DIR": str(tmp_path), "OAL_MEMORY_ENABLED": "false"}
    proc = subprocess.run(
        [sys.executable, str(HOOK)],
        input=json.dumps({}),
        capture_output=True, text=True,
