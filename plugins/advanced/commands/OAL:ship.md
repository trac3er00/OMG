...
- Consume plan artifacts generated by `/OAL:deep-plan`.
- Require `.oal/state/_plan.md` and `.oal/state/_checklist.md` to already encode the business workflow contract.
- If missing, invoke `/OAL:deep-plan` first and return after those artifacts are created.
