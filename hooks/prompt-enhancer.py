...
try:
    ...
    try:
        detect_codex_available = importlib.import_module(
            "_common"
        ).detect_codex_available
...
prompt = data.get("tool_input", {}).get("user_message", "") or data.get(
    "user_message", ""
)
if not prompt:
    sys.exit(0)
...
def inject_context(current: list[str], text: str, max_chars: int) -> list[str]:
    remaining = max_chars - sum(len(i) for i in current)
    if remaining <= 20:
        return current
    candidate = text
    if len(candidate) > remaining:
        candidate = candidate[: remaining - 3] + "..."
    return [*current, candidate]


def add(text):
    global injections
    injections = inject_context(injections, text, MAX_CHARS)


def classify_intent(
    prompt_text: str, intent_map: Mapping[str, Mapping[str, object]]
) -> str | None:
    for intent_key, intent_data in intent_map.items():
        signals = intent_data.get("signals", [])
        if isinstance(signals, list) and any(
            signal_matches_text(str(sig), prompt_text) for sig in signals
        ):
            return intent_key
    return None


def route_mode(prompt_text: str) -> tuple[bool, bool]:
    ulw_signals = [
        "ulw",
        "ultrawork",
        "ralph",
    ...
    is_ulw_mode = any(signal_matches_text(sig, prompt_text) for sig in ulw_signals)
    is_crazy_mode = any(signal_matches_text(sig, prompt_text) for sig in crazy_signals)
    return is_ulw_mode, is_crazy_mode


def emit_output(context_injections: list[str], max_chars: int) -> str:
    if not context_injections:
        return ""
...
_has_any_signal = any(
    [
        any(
            signal_matches_text(sig, prompt)
            for sig in [
                "fix",
                "bug",
                "implement",
...
def _build_intent_gate_state(prompt_text, payload):
    ...
    has_pref = (
        _count_signals(INTENT_MAP["preference_memory"]["signals"], prompt_text) > 0
    )
    has_ambiguous_config = (
        _count_signals(INTENT_MAP["ambiguous_config"]["signals"], prompt_text) > 0
    )
    if not (has_auth or has_pref or has_ambiguous_config):
        return None

    has_specific_action = any(
        signal_matches_text(sig, prompt_text) for sig in IMPLEMENTATION_ACTION_SIGNALS
    )
    ...
    if intent_class in ("auth_setup", "ambiguous_config"):
        if not any(
            signal_matches_text(sig, prompt_text) for sig in AUTH_PROVIDER_SIGNALS
        ):
            missing_slots.append("provider")
        if not any(
            signal_matches_text(sig, prompt_text) for sig in AUTH_SOURCE_SIGNALS
        ):
            missing_slots.append("credential_source")
    if intent_class in ("preference_memory", "ambiguous_config"):
        if not any(
            signal_matches_text(sig, prompt_text) for sig in PREFERENCE_SCOPE_SIGNALS
        ):
    ...
    confidence = min(
        ...
        0.76
        + (signal_hits * 0.07)
        + (0.06 if intent_class == "ambiguous_config" else 0.0),
    )
    governance_payload = {}
    try:
        from runtime.complexity_scorer import score_complexity
...
detected_intent = classify_intent(prompt, INTENT_MAP)

intent_gate_state = _build_intent_gate_state(prompt, data)
if intent_gate_state is not None:
    try:
        _write_intent_gate_artifact(
            state_dir, intent_gate_state["run_id"], intent_gate_state
        )
    except Exception:
        try:
            import sys
            ...
            print(
                f"[omg:warn] [prompt-enhancer] failed to write intent gate artifact: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
...
if detected_intent in ("fix", "implement", "refactor"):
    parts.append("@verify: After EVERY change run build/lint/test. Show exit code.")
...
if score_complexity is not None and detected_intent in (
    ...
    try:
        ...
        try:
            ...
            print(
                f"[omg:warn] [prompt-enhancer] failed to compute governance payload: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
...
# ═══════════════════════════════════════════════════════════
is_ulw, is_crazy = route_mode(prompt)
    ...
    if not os.path.exists(ralph_path):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] failed to persist ralph state: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
...
if not is_crazy and not is_ulw and budget_ok():
    ...
    complexity_score += sum(
        2 for s in MULTI_COMPONENT if signal_matches_text(s, prompt)
    )
    ...
    if score_complexity is not None:
        try:
            ...
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] complexity scoring failed: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
    ...
    if (
        get_feature_flag("metaprompt_auto", True)
    ...
    if (
        get_feature_flag("prompt_compiler", True)
        ...
        try:
            ...
            if _task_type in ("fix",):
                ...
                _task_type = "feature"
            _compiled = get_compiled_preamble(
                _task_type, project_dir=_resolve_project_dir()
            )
            if _compiled:
                add(_compiled)
        except Exception:
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] prompt compiler preamble failed: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
...
if os.path.exists(_mode_path) and budget_ok():
    try:
        ...
        try:
            ...
            print(
                f"[omg:warn] [prompt-enhancer] failed to read cognitive mode: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
...
has_deep_plan_signal = any(
    signal_matches_text(sig, prompt) for sig in DEEP_PLAN_SIGNALS
)
...
if not route_lock and get_feature_flag("agent_registry") and budget_ok():
    try:
        ...
        _routing_kws = (
            _maybe_kws if _maybe_kws else set(_ASCII_WORD_RE.findall(prompt_lower))
        )
        ...
        if isinstance(matched_agent, dict):
            ...
            if _preferred == "gemini-cli":
                add(
                    f'@agent: {_agent_name} → /OMG:escalate gemini "[task]" (visual/frontend domain)'
                )
            elif _preferred == "codex-cli":
                add(
                    f'@agent: {_agent_name} → /OMG:escalate codex "[task]" (backend/security domain)'
                )
        ...
        try:
            ...
            print(
                f"[omg:warn] [prompt-enhancer] agent registry routing failed: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
...
if (
    any(signal_matches_text(sig, prompt) for sig in SEQUENTIAL_THINKING_SIGNALS)
    and budget_ok()
):
    add(
        "@reasoning: Use /OMG:sequential-thinking for structured hypothesis and verification flow."
    )
...
if (
    not route_lock
    and any(signal_matches_text(sig, prompt) for sig in SECURITY_SIGNALS)
    and budget_ok()
):
    if detected_intent in ("fix", "implement", "refactor"):
        add(
            "@security: CRITICAL DOMAIN — No hardcoded secrets. Run /OMG:security-check after."
        )
...
if any(signal_matches_text(sig, prompt) for sig in RESUME_SIGNALS) and budget_ok():
    for hp in [
        os.path.join(state_dir, "handoff.md"),
        os.path.join(state_dir, "handoff-portable.md"),
    ]:
        if os.path.exists(hp):
            try:
                ...
                try:
                    ...
                    print(
                        f"[omg:warn] [prompt-enhancer] handoff extraction failed: {sys.exc_info()[1]}",
                        file=sys.stderr,
                    )
                except Exception:
                    pass
    ...
    if os.path.exists(cp):
        try:
            ...
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] checklist progress read failed: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
        ...
        try:
            if os.path.exists(index_path):
                ...
                if not isinstance(index, dict):
                    print(
                        f"[OMG] prompt-enhancer: index.json is not a dict ({type(index).__name__}), rebuilding",
                        file=sys.stderr,
                    )
                    try:
                        ...
                        try:
                            ...
                            print(
                                f"[omg:warn] [prompt-enhancer] failed to remove invalid knowledge index: {sys.exc_info()[1]}",
                                file=sys.stderr,
                            )
                        except Exception:
                            pass
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"[omg:warn] [prompt-enhancer] failed to delete corrupted knowledge index: {sys.exc_info()[1]}",
                        file=sys.stderr,
                    )
                except Exception:
                    pass
            ...
            for fn in files:
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            f"[omg:warn] [prompt-enhancer] failed to index knowledge file: {sys.exc_info()[1]}",
                            file=sys.stderr,
                        )
                    except Exception:
                        pass
...
if get_feature_flag("memory") and budget_ok():
    try:
        ...
        if _mem_kws:
            mem_context = search_memories(
                project_dir, _mem_kws, max_results=3, max_chars=200
            )
            if mem_context:
        ...
        try:
            ...
            print(
                f"[omg:warn] [prompt-enhancer] memory retrieval failed: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
    ...
    if os.path.exists(tp):
        try:
            ...
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] failure tracker read failed: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
    ...
    if active:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[omg:warn] [prompt-enhancer] failed to parse stuck timestamp: {sys.exc_info()[1]}",
                    file=sys.stderr,
                )
            except Exception:
                pass
        if should_inject:
            try:
                ...
                try:
                    ...
                    print(
                        f"[omg:warn] [prompt-enhancer] failed to persist stuck timestamp: {sys.exc_info()[1]}",
                        file=sys.stderr,
                    )
                except Exception:
                    pass
            add(
                f"@stuck{ctx}: STOP retrying. /OMG:escalate codex | different approach | ask user"
            )
...
_ctx_engine_packet_path = os.path.join(
...
# ═══════════════════════════════════════════════════════════
if (
    get_feature_flag("tool_discovery", True)
    ...
    try:
        ...
        try:
            ...
            print(
                f"[omg:warn] [prompt-enhancer] semantic tool discovery failed: {sys.exc_info()[1]}",
                file=sys.stderr,
            )
        except Exception:
            pass
...
try:
    ...
    if is_high_pressure:
        add(
            f"@context-pressure: High context usage detected ({tool_count} tool calls). Auto-saving state..."
        )
except Exception:
    try:
        import sys
        ...
        print(
            f"[omg:warn] [prompt-enhancer] context pressure estimation failed: {sys.exc_info()[1]}",
            file=sys.stderr,
        )
    except Exception:
        pass
...
if injections:
    output = emit_output(injections, MAX_CHARS)
