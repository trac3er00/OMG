...
from runtime.config_transaction import ConfigTransactionError  # noqa: E402
...
def configure_mcp(
    ...
    with transactional() as tx:
        try:
            ...
            claude_content = json.dumps(claude_config, indent=2, ensure_ascii=True) + "\n"
            tx.plan(Path(config_path), claude_content)
        except Exception as exc:
        ...
        try:
            ...
            _logger.warning("Config transaction failed, all writes rolled back: %s", exc)
            for host in configured:
                errors.setdefault(host, f"rolled back: {exc}")
            if "claude" not in errors:
                errors["claude"] = f"rolled back: {exc}"
            configured = []
